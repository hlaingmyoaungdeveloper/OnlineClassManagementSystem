...
Client submits class details (ClassName, Place, OpenDate, OpenTime, StudentLimit).
SubClassService validates required fields and checks for duplicate class names or overlapping place/date/time.
Class is saved to Tbl_SubClass with initial StudentCount = 0.
2. Enrollment Workflow
Client requests enrollment with ClassId and StudentId.
EnrollmentService verifies:
...
Adds record to Tbl_Enrollments and automatically increments StudentCount in Tbl_SubClass.
3. Enrollment Cancellation / Soft Delete Workflow
Client requests deletion of an enrollment by EnrollmentId.
EnrollmentService marks Tbl_Enrollments.IsDelete = true.
Automatically decrements StudentCount in Tbl_SubClass.
✨ Features List
🏫 SubClass Management
...
✅ Implementation Checklist
 Solution setup with Clean Architecture layout (.Database, .Domain, .WebApi)
 Entity Framework Core models & DbContext configuration (AppDbContext)
 SubClass Feature
 SubClassService implementation
 Request/Response models (SubClassList, SubClassEdit, SubClassCreate, SubClassPatch, SubClassDelete)
 SubClassController endpoints (GET, POST, PATCH, DELETE)
 Enrollment Feature
 EnrollmentService implementation
 Request/Response models (EnrollmentList, EnrollmentEdit, EnrollmentCreate, EnrollmentPatch, EnrollmentDelete)
 EnrollmentController endpoints (GET, POST, PATCH, DELETE)
 Automatic capacity check & StudentCount synchronization
 Dependency Injection configuration in Program.cs
 Solution build verification (0 Errors)
